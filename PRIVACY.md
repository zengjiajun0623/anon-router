...
| **Transport** | Does the network path link you? | **Onion live.** The router is reachable as a Tor v3 `.onion`, so you can connect without revealing an IP and with no exit node in the path. Over clearnet the app itself does not log IPs (`--no-access-log`), but the hosting platform's edge terminates TLS and can see request IPs in its own logs; the `.onion` bypasses that edge. Inference is paid with blind-signed ecash that carries no key, so there is no per-key request linkage; timing and amounts still correlate, so space out claims and spends. |
...
- **Stateless, sessionless auth.** Inference is paid with blind-signed ecash (or a
  per-request channel payment) in headers, with no bearer key on the inference path,
  no cookies, no login, no server session that links requests.
- **No app-level IP logging.** Run with `uvicorn --no-access-log`; the app never reads
  `X-Forwarded-For` or stores client IPs, and responses are `no-store`. The hosting
  platform's edge can still log request IPs; connect over the `.onion` to bypass it.
